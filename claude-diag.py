...
type Publisher = Callable[[str, str], str]
...
def section_footer(
    ...
    if (
        ...
        and (usage_summary.tokens.total or 0) > 0
    ):
        lines.append(format_usage_summary(usage_summary))
    lines.extend(
        [
...
def self_test() -> int:
    ...
    final_flow_failures = _self_test_final_flow()
    print("=== checks ===")
    if failures:
    ...
    for failure in prompt_failures:
        print(f"FAIL: {failure}")
    for failure in final_flow_failures:
        print(f"FAIL: {failure}")
    ok = (
        not failures
        and not missing
...
def _self_test_context() -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="claude-diag-test-") as tmp:
        ...
        try:
            ...
            if f"Repository: {REPOSITORY_URL}" not in footer:
                failures.append("repository URL was not shown in footer")
            if "cdiag.link/claude-diag.py" in footer or "Source:" in footer:
                failures.append("old source URL was still shown in footer")

            zero_footer = section_footer(
                r,
                UsageSummary(0.01, TokenUsage(input_tokens=0, output_tokens=0)),
                False,
            )
            if "Run usage:" in zero_footer or "$0.01" in zero_footer:
                failures.append("zero-token usage was not suppressed in footer")

            unavailable_footer = section_footer(r, None, False)
            if "Run usage:" in unavailable_footer:
                failures.append("unavailable usage was not suppressed in footer")
            ...
            if "Run usage:" in footer:
                failures.append(
                    "--no-context footer did not suppress unavailable usage"
                )
        finally:
...
def _final_flow_args(
    ...
    *,
    ...
    args.publish_expiry = "1w"
    return args


def _self_test_final_flow() -> list[str]:
    failures: list[str] = []
    report = "## report\nbody\n"

    with tempfile.TemporaryDirectory(prefix="claude-diag-flow-") as tmp:
        tmp_path = Path(tmp)

        publish_calls: list[tuple[str, str]] = []

        def fake_publisher(published_report: str, expiry: str) -> str:
            publish_calls.append((published_report, expiry))
            return "https://paste.myst.rs/fake123"

        args = _final_flow_args(tmp_path / "declined.md")
        stderr = io.StringIO()
        stdout = io.StringIO()
        code = finish_report(
            report,
        ...
        wrote_i = rendered.find("[claude-diag] wrote")
        publish_i = rendered.find("Publish redacted report")
        print_i = rendered.find("Print report")
        cat_i = rendered.find("cat ")
        if code != 0:
            failures.append(f"publish declined flow returned {code}, expected 0")
        if publish_calls:
            failures.append("publish declined flow called publisher")
        if stdout.getvalue():
            failures.append("output declined flow wrote report to stdout")
        if not (0 <= wrote_i < publish_i < print_i < cat_i):
            failures.append("final prompts were not ordered save, publish, print, cat")

        publish_calls.clear()
        args = _final_flow_args(tmp_path / "accepted.md")
        stderr = io.StringIO()
        stdout = io.StringIO()
        code = finish_report(
            report,
        ...
        if code != 0:
            failures.append(f"publish accepted flow returned {code}, expected 0")
        if publish_calls != [(report, "1w")]:
            failures.append(f"publish accepted calls mismatch: {publish_calls}")
        if "https://paste.myst.rs/fake123 (expires 1w)" not in stderr.getvalue():
            failures.append("publish accepted flow did not show URL and expiry")
        if stdout.getvalue():
            failures.append("publish accepted output declined flow wrote stdout")

        publish_calls.clear()
        args = _final_flow_args(tmp_path / "default-publish.md", publish=True)
        code = finish_report(
            report,
            args,
            input_stream=io.StringIO("\n\n"),
        ...
        if code != 0:
            failures.append(f"default publish flow returned {code}, expected 0")
        if publish_calls != [(report, "1w")]:
            failures.append("interactive --publish did not default to yes")

        publish_calls.clear()
        args = _final_flow_args(tmp_path / "printed.md")
        stderr = io.StringIO()
        stdout = io.StringIO()
        code = finish_report(
            report,
        ...
        if code != 0:
            failures.append(f"output accepted flow returned {code}, expected 0")
        if stdout.getvalue() != report:
            failures.append("output accepted flow did not print report")
        if "cat " in stderr.getvalue():
            failures.append("output accepted flow still printed cat command")

        args = _final_flow_args(tmp_path / "no-save.md", no_save=True)
        stdout = io.StringIO()
        code = finish_report(
            report,
            args,
        ...
        if code != 0:
            failures.append(f"--no-save flow returned {code}, expected 0")
        if stdout.getvalue() != report:
            failures.append("--no-save flow did not preserve stdout-only output")
...
def build_report(
    ...
    *,
    ...
    def add(label: str, build: Callable[[], str]) -> None:
        nonlocal step
        step += 1
        write_progress(step, total, label, progress_stream)
        sections.append(build())

    add("header", section_header)
    add("environment", lambda: section_environment(redact, args.timeout))
    add("claude command", lambda: section_claude(redact, args.timeout))

    step += 1
    write_progress(step, total, "/context output", progress_stream)
    context_section, usage_summary = section_context(
        redact, args.model, args.timeout, args.no_context
    )
    sections.append(context_section)

    add("global settings", lambda: section_global_settings(redact))
    add("project settings", lambda: section_project_settings(redact))
    add("mcp servers", lambda: section_mcp(redact, args.timeout))
    add("plugins", lambda: section_plugins(redact, args.timeout))
    add("skills", lambda: section_skills(redact))
    add("agents", lambda: section_agents(redact, args.include_memories))
    add("commands", lambda: section_commands(redact))
    add("memories", lambda: section_memories(redact, args.include_memories))
    add("hooks", lambda: section_hooks(redact))
    add("state footprint", lambda: section_state_footprint(redact))
    add("activity", lambda: section_activity(redact))
    add("recent errors", lambda: section_recent_errors(redact))
    add("about", lambda: section_footer(redact, usage_summary, args.no_context))
    return redact("\n".join(sections))


def default_output_path() -> str:
    ts = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
    return f"/tmp/claude-diag-{ts}.md"
...
def write_report_stdout(report: str, stdout_stream: TextIO = sys.stdout) -> None:
    _ = stdout_stream.write(report)
    if not report.endswith("\n"):
        _ = stdout_stream.write("\n")
...
def publish_report(
    ...
    expiry: str,
    *,
    publisher: Publisher = publish_to_pastemyst,
    output_stream: TextIO = sys.stderr,
) -> bool:
    try:
...
def finish_report(
    ...
    *,
    ...
    saved_path = save_report(report, args, output_stream=output_stream)

    if args.yes:
        write_report_stdout(report, stdout_stream)
        if args.publish and not publish_report(
            report,
            args.publish_expiry,
            publisher=publisher,
    ...
    if prompt_confirm(
        f"Publish redacted report to PasteMyst (expires {args.publish_expiry})?",
